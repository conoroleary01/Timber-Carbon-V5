...
const A4_DISTANCE_MULTIPLIER = 1.43;
const A4_EMISSION_FACTOR = 0.1211;
...
function validateMaterial(line: MaterialLineInput) {
  ...
  // force automatic derivation/validation to run
...
export function calculateProject(input: ProjectInput): CalculationResult {
  ...
  assertNonNegative("endOfLife.c1KgCO2e", input.endOfLife.c1KgCO2e);
  ...
  const finishedProductA1A3KgCO2e =
    ...
    (input.factory.a4DistanceKm * A4_DISTANCE_MULTIPLIER) *
    A4_EMISSION_FACTOR;
...
const A5DemolitionKgCO2e = 17.5 * input.giaDemolishedM2;
const A5ConstructionKgCO2e = 20 * input.reportingAreaM2;
const A5PercentOfA1A4KgCO2e = totalA1ToA4KgCO2e * 0.01;
...
const upfrontCarbonKgCO2e =
  ...
  const C1KgCO2e = input.endOfLife.c1KgCO2e;

  const C2KgCO2e = sum(
    input.endOfLife.routes.map((route) => {
      assertNonNegative(`${route.name} massTonnes`, route.massTonnes);
      assertNonNegative(`${route.name} distanceKm`, route.distanceKm);
      assertNonNegative(
        `${route.name} transportKgCO2ePerTkm`,
        route.transportKgCO2ePerTkm,
      );
      return route.massTonnes * route.distanceKm * route.transportKgCO2ePerTkm;
    }),
  );
  ...
  const C3KgCO2e = sum(
    input.endOfLife.routes.map((route) => {
      assertNonNegative(
        `${route.name} processingKgCO2ePerTonne`,
        route.processingKgCO2ePerTonne,
      );
      return route.massTonnes * route.processingKgCO2ePerTonne;
    }),
  );

  const C4KgCO2e = sum(
    input.endOfLife.routes.map((route) => {
      assertNonNegative(`${route.name} disposedMassTonnes`, route.disposedMassTonnes);
      assertNonNegative(
        `${route.name} disposalKgCO2ePerTonne`,
        route.disposalKgCO2ePerTonne,
      );
      return route.disposedMassTonnes * route.disposalKgCO2ePerTonne;
    }),
  );
 ...
 const embodiedCarbonTotalKgCO2e =
  ...
  upfrontCarbonKgCO2e + C1KgCO2e + C2KgCO2e + C3KgCO2e + C4KgCO2e,
